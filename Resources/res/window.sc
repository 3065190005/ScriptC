require ("extern_system\\system_window");
...
interface StdWindow{
	...
	function msgBox(title,txt,type):
		return __system__window__msgBox__(title,txt,type);
	end

	// title|str , txt|str -> boolean|str;
	function editBox(title,txt):
		return __system__window__editBox__(title,txt);
	end


	// (void) -> boolean|str;
	function bowserBox():
		return __system__window__bowserBox__();
	end

	// title|str , html|str -> boolean;
	function htmlBox(title,html):
		return __system__window__htmlBox__(title,html);
	end

	// title|str , url|str -> boolean;
	function urlBox(title,url):
		return __system__window__urlBox__(title,url);
	end

	// title|str , html|str -> boolean;
	function htmlView(title,html):
		return __system__window__htmlView__(title,html);
	end

	// title|str , url|str -> boolean;
	function urlView(title,url):
		return __system__window__urlView__(title,url);
	end

	//----------
	// (void) -> array;
	function getWebBoxSize():
		return __system__window__getWebBoxSize__();
	end

	// x|number, y|number, width|number, height|number -> boolean;
	function setWebBoxSize(x,y,width,height):
		return __system__window__setWebBoxSize__(x,y,width,height);
	end

	// (void) -> number;
	function getWebBoxStyle():
		return __system__window__getWebBoxStyle__();
	end

	// style|number -> boolean;
	function setWebBoxStyle(style):
		return __system__window__setWebBoxStyle__(style);
	end
